...
public interface TutorialStepRepository extends CrudRepository <TutorialStepEntity,Integer> {
    ...
    @Query(value = " SELECT * FROM TUTORIAL_STEP WHERE STATUS = :status ", nativeQuery = true)
    List<TutorialStepEntity> findByStatus(@Param("status") String status);

    @Query(value = " SELECT * FROM TUTORIAL_STEP WHERE CODE = :code and STATUS = :status ", nativeQuery = true)
